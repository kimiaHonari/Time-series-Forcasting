...
def get_callbacks():
    model_type = "LSTM"
    save_dir = os.path.join(os.getcwd(), 'LSTM_co')
...
model = Sequential()
# model.add(layers.Embedding(input_shape=(lookback // step, float_data.shape[-1])))
model.add(layers.LSTM(120,input_shape=(1,319)))
